...
import PieGraph from '../../components/general_components/ChartPie.js';
...
function DailyTimedCreateActivity(props) {
    ...
    const get_daily_habit_totals = async () => {
        ...
        let getResponse = await fetch(`https://shrouded-ravine-06737.herokuapp.com/habits/daily_habits/${dailyHabitId}`, {
    ...
    const get_date_habit_totals = async () => {
        ...
        let jsonResponse = await getResponse.json()
        const [hours, minutes] = jsonResponse.accumulated_time ? formatTimedAmountAccumulated(jsonResponse.accumulated_time) : [0, 0]
    ...
    const updateActivity = async (activity_id) => {
        ...
        let token = localStorage.getItem('token')
        let putResponse = await fetch(`https://shrouded-ravine-06737.herokuapp.com/habits/activities/update/${activity_id}/${year}/${month}/${day}/${hours}/${minutes}/${seconds}/`, {
    ...
    const goal_information = () => {
        ...
        // the goal text 
        let habit_text = <h5> Today's goal: {habitData.goal_amount} hours </h5>
    ...
    return (
        <div>
            <Container>
                    ...
                    <Col>
                        <Card style={{ width: '100%' }}>
                            <Card.Body className={habitData && dailyHabitData ? habitData.type_of_goal == 'total' ? habitData.completed ? 'greenCard' : 'redCard' : dailyHabitData.completed ? 'greenCard' : 'redCard' : null}>
                                <Card.Subtitle className="mb-4 align-text-left  d-flex ">{habitData && dailyHabitData ? <h5 className={habitData.type_of_goal == 'total' ? habitData.completed ? 'greenTitle' : 'redTitle' : dailyHabitData.completed ? 'greenTitle' : 'redTitle'}> Habit Activity: {habitData.title}</h5> : null} {/*make sure data loaded*/}</Card.Subtitle>
                                ...
                                <Card.Subtitle className="mb-4 align-text-left  d-flex">{habitData ? goal_information() : null}{/*make sure data loaded*/}</Card.Subtitle>
                                <Card.Subtitle className="mb-4 align-text-left  d-flex">{habitData ? <h5> Hrs: {totalAccumulatedTimeHours} Minutes: {totalAccumulatedTimeMinutes} </h5> : null} </Card.Subtitle>
                                {/* only allow access to create activity buttons if today is the same day as activity habit selected */}
                                {allowAtivityCreation ?
                                    <div class='timed'> <Button style={{ width: '100%' }} className="mb-4 align-text-left  d-flex btn btn-success" onClick={createOrStartActivity}> Start New Activity </Button><br></br></div> : null}
                ...
                {totalAccumulatedTimeHours && habitData ? <PieGraph title={habitData.title} completed={(parseFloat(totalAccumulatedTimeHours) * 60) + parseFloat(totalAccumulatedTimeMinutes)} total={habitData.goal_amount * 60} /> : null}
